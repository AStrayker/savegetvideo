    ...
    try:
        ...
        if FIXIE_SOCKS_HOST:
            ...
            try:
                ...
                print(f"Error downloading video (with proxy): {e}")
                return None
    ...
    try:
        ...
        if FIXIE_SOCKS_HOST:
            ...
            try:
                ...
                print(f"Error downloading audio (with proxy): {e}")
                return None
...
async def getIds(video):
    ...
    with YoutubeDL({'quiet': True}) as ydl:
        ...
        try:
            ...
            ids.extend(
                [x.get('id'), x.get('playlist_index'), x.get('creator') or x.get('uploader'), x.get('title'),
                 x.get('duration'), x.get('thumbnail')] for x in info_dict)
        except:
            ids.append(
                [info_dict.get('id'), info_dict.get('playlist_index'), info_dict.get('creator') or info_dict.get('uploader'),
                 info_dict.get('title'), info_dict.get('duration'), info_dict.get('thumbnail')])
    return ids


@Mbot.on_message(filters.regex(r'https?://.*youtube[^\s]+') & filters.incoming | filters.regex(
    r'(https?:\/\/(?:www\.)?youtu\.?be(?:\.com)?\/.*)') & filters.incoming)
async def _(Mbot, message):
    ...
    if "shorts" in link:
        try:
            ...
            if LOG_GROUP:
                ...
                await message.reply(
                    f"400: Sorry, Unable To Find It try another or report it to @masterolic or support chat @spotify_supportbot 🤖 ")
    ...
    try:
        ...
        for id in ids:
            ...
            PForCopy = await message.reply_photo(f"https://i.ytimg.com/vi/{id[0]}/hqdefault.jpg",
            ...
            if fileLink:
                ...
                AForCopy = await message.reply_audio(fileLink,
                                                    caption=f"[{id[3]}](https://youtu.be/{id[0]}) - {id[2]} Thank you for using - @InstaReelsdownbot",
                                                    title=id[3].replace("_", " "), performer=id[2], thumb=thumnail,
                                                    duration=id[4])
                if DUMP_GROUP:
